...
class load_data:
    ...
    @classmethod
    def rois_spec(cls, data, flims, ch_rec, db, bstd, bp):   
        #target_fs=48000
        #s = sound.resample(s, fs, target_fs, res_type='kaiser_fast')
        s_filt=data.wav    
    ...
    def ac_print(cls, df):
        ...
        for site, data in group_site:        
            ...
            # Save a figure per site with a safe filename
            safe_site = ''.join(c if (c.isalnum() or c in '._- ') else '_' for c in str(site))
            out_name = f"site_print_{safe_site}.png"
...
longwave = load_data.longwave
shortwave = load_data.shortwave
rois_spec = load_data.rois_spec
ind_batch = load_data.ind_batch
ind_per_day = load_data.ind_per_day
saturation = load_data.saturation
compute_acoustic_indices = load_data.compute_acoustic_indices
plot_acoustic_indices = load_data.plot_acoustic_indices
spl_batch = load_data.spl_batch
plot_spl = load_data.plot_spl
ac_print = load_data.ac_print
