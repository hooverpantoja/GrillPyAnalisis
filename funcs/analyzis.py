...
class Analyzer:
    ...
    def ac_print(df):
        ...
        # create a new Figure and Axes for each site to avoid reusing figures
            fig, ax = plt.subplots()
            ax.set_ylabel('Hour')
            ax.set_xlabel('Frequency (kHz)')
            ax.set_title(site)
            # use Matplotlib's Axes.imshow to plot the downscaled spectrogram
            ax.imshow(Sd, aspect='auto', origin='lower', extent=[tn[0], tn[len(tn)-1], 0, fn[-1]/1000])
            Ssites[:, :, k] = Sd
            ...
            # First figure: annotated scatter with image thumbnails
            ...
            for x, ind in zip(X, range(pts.shape[0])):
                ...
                ax2.add_artist(ab)
            ax2.set_title('Metric MDS with Euclidean')
            ax2.set_ylabel('NMDSy')
            ax2.set_xlabel('NMDSx')
            plt.show(block=False)

            # Second figure: colored scatter by site
            fig3, ax3 = plt.subplots(figsize=(15, 6))
            sns.scatterplot(x=pts[:, 0], y=pts[:, 1], hue=sites_ind, palette='pastel', ax=ax3)
            ax3.set_title('Metric MDS with Euclidean')
            ax3.set_ylabel('NMDSy')
            ax3.set_xlabel('NMDSx')
            plt.show(block=False)
        return(X,y,pts)               
